...
scatter.follower_repost  = ggplot(data.long.filted, aes(x = followers, y = repost_count, color = keyword)) +
  geom_point(alpha = 0.5,size = 2)+
  scale_y_continuous(trans = log10_trans())+
  scale_x_continuous(trans = log10_trans())

scatter.follower_repost

scatter.company.follower_repost  = ggplot(data.long.filted, aes(x = followers, y = repost_count, color = keyword)) +
  geom_point(alpha = 0.5,size = 1)+
  facet_grid(keyword~. , scale = "free")+
  scale_y_continuous(trans = log10_trans())+
  scale_x_continuous(trans = log10_trans())
scatter.company.follower_repost
...
prize.data = read.csv("./coin.csv", sep = ",")
BTC = prize[1:6,]
ETH = prize[7:12,]
BCH = prize[13:18,]
